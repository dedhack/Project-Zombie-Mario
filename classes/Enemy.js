    ...
    for (let i = 0; i < this.collisionBlocks.length; i++) {
      ...
      if (collision({ object1: this.hitbox, object2: collisionBlock })) {
        ...
        if (this.velocity.y > 0) {
          this.velocity.y = 0;
          // to ensure that the sprite is positioned on top of the top of the collision block
          const offset =
        ...
        // For collisions with the bottom of a collision block when sprite character is jumping
        if (this.velocity.y < 0) {
          this.velocity.y = 0;
