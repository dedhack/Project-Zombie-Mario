...
class Player extends Sprite {
  constructor({
    ...
    this.collisionBlocks = collisionBlocks; // pass to player object collisionblocks 2D array
  ...
  checkForHorizontalCollisions() {
    for (let i = 0; i < this.collisionBlocks.length; i++) {
      ...
      if (collision({ object1: this.hitbox, object2: collisionBlock })) {
        // movement to the right and check for collision blocks
        ...
        // movement to the left and check for collisions blocks
        if (this.velocity.x < 0) {
          this.velocity.x = 0;
    ...
    for (let i = 0; i < this.collisionBlocks.length; i++) {
      ...
      if (collision({ object1: this.hitbox, object2: collisionBlock })) {
        ...
        if (this.velocity.y > 0) {
          ...
          this.position.y = collisionBlock.position.y - offset - 0.01; // update on the this.height -0.01 to ensure that the block is not colliding anymore. will be used for horizontal collision check
          break;
        }
        ...
        if (this.velocity.y < 0) {
          ...
          this.position.y =
            collisionBlock.position.y + collisionBlock.height - offset + 0.01; // +0.01 to ensure that the block is not colliding anymore. will be used for horizontal collision check
          break;
        }
      }
    }
